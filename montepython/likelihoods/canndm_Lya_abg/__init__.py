...
class canndm_Lya_abg(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        if os.path.exists(file_path):
            try:
                ...
                pkl = open(file_path, 'rb')
                self.input_full_matrix_interpolated_ABG = pickle.load(pkl, encoding='latin1')
            pkl.close()
        else:
    ...
    def loglkl(self, cosmo, data):
        ...
        if (sigma8<self.zind_param_min[1] or sigma8>self.zind_param_max[1]):       #FRA 
        ...
        if 'xi_idr' in data.cosmo_arguments or 'N_ur' in data.cosmo_arguments or 'omega_canndm' in data.cosmo_arguments or 'N_dg' in data.cosmo_arguments or 'T_ncdm' in data.cosmo_arguments:
            ...
            if 'T_ncdm' in data.cosmo_arguments and ('omega_ncdm' in data.cosmo_arguments or 'Omega_ncdm' in data.cosmo_arguments):
                ...
                if 'omega_ncdm' in data.cosmo_arguments:
                    ...
                    del data.cosmo_arguments['omega_ncdm']
                remove = ['m_ncdm','aNR','N_ncdm','T_ncdm','ncdm_fluid_approximation','l_max_ncdm','ncdm_psd_parameters','Maximum q','Quadrature strategy']
                for bad in remove:
            ...
            if 'xi_idr' in data.cosmo_arguments or 'N_idr' in data.cosmo_arguments or 'N_dg' in data.cosmo_arguments:
                ...
                for bad in to_remove:
                    if bad in data.cosmo_arguments:
                        del data.cosmo_arguments[bad]
            # Deal with Warm Dark Matter
            if 'omega_canndm' in data.cosmo_arguments:
                if 'omega_canndm' in data.cosmo_arguments:
                    if 'Omega_cdm' in data.cosmo_arguments:
                        data.cosmo_arguments['Omega_cdm'] +=data.cosmo_arguments['omega_canndm']/h/h
                    if 'omega_cdm' in data.cosmo_arguments:
                        data.cosmo_arguments['omega_cdm'] +=data.cosmo_arguments['omega_canndm']/eta2
                    del data.cosmo_arguments['omega_canndm']
                remove = ['aNR','perfectfluid','l_max_scalars','lensing']
                for bad in remove:
                    if bad in data.cosmo_arguments:
                        del data.cosmo_arguments[bad]
        ...
        cosmo.empty()
        data.cosmo_arguments['omega_canndm']=0
        ...
        #print('Canndm_plin='+str(Plin[8517])+' Equiv_plin='+str(Plin_equiv[8517]),' at k='+str(k[8517]))
        ...
        #FRA disegna
        #plt.loglog(k,Plin_equiv)
        #plt.loglog(k,Plin,'r-')
        #plt.axvline(k_eq)
        #plt.savefig('chains/mpk.pdf')
        #end FRA


       # if any(abs(Tk[k<np.maximum(k_eq,k[0])]**2-1.0)>0.01):
        if any(abs(Tk[k<np.maximum(k_eq,k[0])]**2-1.0)>0.1):
            with open(self.bin_file_path, 'a') as bin_file:
        ...
        #print('FIT: alpha='+str(best_alpha)+' beta='+str(best_beta)+' gamma='+str(best_gamma))
        ...
        #FRA    
        #print('kmax='+str(k[index_k_fit_max])+' at index '+str(index_k_fit_max))

        #plt.figure(3);plt.xscale('log');plt.yscale('log');plt.xlim(k[0],k[-1]);
        #plt.plot(k_fit,Tk_fit,'k-',label=r'TK')
        #plt.axvline(k[index_k_fit_max])
        #plt.plot(k_fit,Tk_abg,'r-',label=r'TK')
        #plt.savefig('chains/tk.pdf',bbox_inches='tight')
        ...
        for ik in range(len(k_fit)):
            ...
            if Tk_fit[ik]<0.2:
               ...
               break
        if any(abs(Tk_fit[:index_k_check_max]/Tk_abg[:index_k_check_max]-1.)>1): #FRA cambiato 0.1->1
            with open(self.bin_file_path, 'a') as bin_file:
                for elem in data.get_mcmc_parameters(['varying']):
                    bin_file.write(' %.6e\t' % data.mcmc_parameters[elem]['current'])
                for elem in data.get_mcmc_parameters(['derived']):
        ...
        #print('aNR='+str(data.cosmo_arguments['aNR'])+'-> Chi quadro='+str(chi2))#FRA
        loglkl = - 0.5 * chi2
